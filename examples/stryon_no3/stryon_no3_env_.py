...
class StryonNo3Env:
    ...
    def reset_idx(self, envs_idx):
        ...
        # Domain randomization
        self.randomize_domain(envs_idx)
    ...
    def curriculum_commands(self):
        ...
        if self.curriculum_step >= self.curriculum_cfg["curriculum_step"]:
            ...
            # ----- Leg length curriculum -----
            leg_err_high = 999
            if self.curriculum_cfg["err_mode"]:
                self.leg_range_up_threshold = self.curriculum_cfg["leg_err_range"][0]
                leg_err_high = self.curriculum_cfg["leg_err_range"][1]
            else:
                mean_leg_range = self.command_ranges[:, 2, 1].mean()
                ratio = mean_leg_range / self.command_cfg["leg_length_angle_range"][1]
                self.leg_range_up_threshold = (
                    self.curriculum_cfg["leg_err_range"][0]
                    + (self.curriculum_cfg["leg_err_range"][1] - self.curriculum_cfg["leg_err_range"][0]) * ratio
                )
                leg_err_high = self.curriculum_cfg["leg_err_range"][2]

            if self.survive_ratio > 0.9:
                old_leg_range = self.command_ranges[0, 2].clone().cpu().numpy()
                if self.mean_leg_error < self.leg_range_up_threshold:
                    # Increase leg_length command range
                    self.command_ranges[:, 2, 0] += (
                        self.curriculum_cfg["curriculum_leg_step"] * self.command_cfg["leg_length_angle_range"][0]
                    )
                    self.command_ranges[:, 2, 1] += (
                        self.curriculum_cfg["curriculum_leg_step"] * self.command_cfg["leg_length_angle_range"][1]
                    )
                    action = "↑ increased"
                elif self.mean_leg_error > leg_err_high:
                    # Decrease leg_length command range
                    self.command_ranges[:, 2, 0] -= (
                        self.curriculum_cfg["curriculum_leg_step"] * self.command_cfg["leg_length_angle_range"][0]
                    )
                    self.command_ranges[:, 2, 1] -= (
                ...
                # Clamp the range within allowed bounds
                self.command_ranges[:, 2, 0] = torch.clamp(
                    self.command_ranges[:, 2, 0],
                    self.curriculum_cfg["curriculum_leg_min_range"] * self.command_cfg["leg_length_angle_range"][0],
                )
                self.command_ranges[:, 2, 1] = torch.clamp(
                    self.command_ranges[:, 2, 1],
                    self.curriculum_cfg["curriculum_leg_min_range"] * self.command_cfg["leg_length_angle_range"][1],
                    self.command_cfg["leg_length_angle_range"][1],
                )

                new_leg_range = self.command_ranges[0, 2].clone().cpu().numpy()
                print(
                    f"[Curriculum] leg_length | error: {self.mean_leg_error:.4f} | "
                    f"range: [{old_leg_range[0]:.3f}, {old_leg_range[1]:.3f}] → "
                    f"[{new_leg_range[0]:.3f}, {new_leg_range[1]:.3f}] | {action}"
                )
    ...
    def domain_rand(self, envs_idx):
        friction_ratio = self.domain_rand_cfg["friction_ratio_range"][0] + (
            self.domain_rand_cfg["friction_ratio_range"][1] - self.domain_rand_cfg["friction_ratio_range"][0]
        ) * torch.rand(len(envs_idx), self.robot.n_links)
        self.robot.set_friction_ratio(
            friction_ratio=friction_ratio, links_idx_local=np.arange(0, self.robot.n_links), envs_idx=envs_idx
        )

        base_mass_shift = self.domain_rand_cfg["random_base_mass_shift_range"][0] + (
            self.domain_rand_cfg["random_base_mass_shift_range"][1]
            - self.domain_rand_cfg["random_base_mass_shift_range"][0]
        ) * torch.rand(len(envs_idx), 1, device=self.device)
        other_mass_shift = -self.domain_rand_cfg["random_base_mass_shift_range"][0] + (
            self.domain_rand_cfg["random_base_mass_shift_range"][1]
            - self.domain_rand_cfg["random_base_mass_shift_range"][0]
        ) * torch.rand(len(envs_idx), self.robot.n_links - 1, device=self.device)
        mass_shift = torch.cat((base_mass_shift, other_mass_shift), dim=1)
        self.robot.set_mass_shift(
            mass_shift=mass_shift, links_idx_local=np.arange(0, self.robot.n_links), envs_idx=envs_idx
        )

        base_com_shift = -self.domain_rand_cfg["random_base_com_shift"] / 2 + self.domain_rand_cfg[
            "random_base_com_shift"
        ] * torch.rand(len(envs_idx), 1, 3, device=self.device)
        other_com_shift = -self.domain_rand_cfg["random_other_com_shift"] / 2 + self.domain_rand_cfg[
            "random_other_com_shift"
        ] * torch.rand(len(envs_idx), self.robot.n_links - 1, 3, device=self.device)
        com_shift = torch.cat((base_com_shift, other_com_shift), dim=1)
        self.robot.set_COM_shift(
            com_shift=com_shift, links_idx_local=np.arange(0, self.robot.n_links), envs_idx=envs_idx
        )

        kp_shift = (
            self.domain_rand_cfg["random_KP"][0]
            + (self.domain_rand_cfg["random_KP"][1] - self.domain_rand_cfg["random_KP"][0])
            * torch.rand(len(envs_idx), self.num_actions, device="cpu")
        ) * self.kp[0]
        self.robot.set_dofs_kp(kp_shift, self.motors_dof_idx, envs_idx=envs_idx)

        kv_shift = (
            self.domain_rand_cfg["random_KV"][0]
            + (self.domain_rand_cfg["random_KV"][1] - self.domain_rand_cfg["random_KV"][0])
            * torch.rand(len(envs_idx), self.num_actions, device="cpu")
        ) * self.kv[0]
        self.robot.set_dofs_kv(kv_shift, self.motors_dof_idx, envs_idx=envs_idx)

        # random_default_joint_angles
        dof_pos_shift = self.domain_rand_cfg["random_default_joint_angles"][0] + (
            self.domain_rand_cfg["random_default_joint_angles"][1]
            - self.domain_rand_cfg["random_default_joint_angles"][0]
        ) * torch.rand(len(envs_idx), self.num_actions, device=self.device, dtype=gs.tc_float)
        self.default_dof_pos[envs_idx] = dof_pos_shift + self.basic_default_dof_pos

        damping = self.domain_rand_cfg["damping_range"][0] + (
            self.domain_rand_cfg["damping_range"][1] - self.domain_rand_cfg["damping_range"][0]
        ) * torch.rand(len(envs_idx), self.robot.n_dofs)
        damping[:, :6] = 0
        self.robot.set_dofs_damping(damping=damping, dofs_idx_local=np.arange(0, self.robot.n_dofs), envs_idx=envs_idx)

        armature = self.domain_rand_cfg["dof_armature_range"][0] + (
            self.domain_rand_cfg["dof_armature_range"][1] - self.domain_rand_cfg["dof_armature_range"][0]
        ) * torch.rand(len(envs_idx), self.robot.n_dofs)
        armature[:, :6] = 0
